       ...
       (with-current-buffer (elpaca-log (concat regexp " | byte-comp | warn\\|error ") t)
         (cl-loop for entry in tabulated-list-entries
                  do (push (string-trim (aref (cadr entry) 2))
                           (melpaca-test-errors melpaca-current-test))))))
   (melpaca-deftest (:title "Package satisfies checkdoc" :syntax 'emacs-lisp)
     (let* ((e (elpaca-get (car (alist-get 'melpaca-recipe pr)))))
       ...
       (require 'checkdoc)
       (with-temp-buffer
         (let ((checkdoc-diagnostic-buffer (buffer-name))
               (inhibit-read-only t)
           ...
           (goto-char (point-min))
           (keep-lines "\\(?:^[[:alnum:]]\\)")
           (if-let ((issues (buffer-substring-no-properties (point-min) (point-max)))
